...
const createDoughnutSeries = (): KChartSeries<SegmentPoint> => {
  type PieSegment = {
    point: SegmentPoint;
    value: number;
    startAngle: number;
  ...
  return createCustomSeries<SegmentPoint>({
    ...
    render({ group, data: points, plotSize }) {
      ...
      group.selectAll<SVGPathElement, PieSegment>('path.doughnut-segment')
        .data(segments)
        .join('path')
        .attr('class', 'doughnut-segment')
      ...
      group.selectAll<SVGTextElement, PieSegment>('text.doughnut-label')
        .data(segments)
        .join('text')
      ...
      group.selectAll<SVGTextElement, string>('text.doughnut-center-title')
        .data(['Total'])
        .join('text')
        .attr('class', 'doughnut-center-title')
        .attr('x', centerX)
        .attr('y', centerY - 6)
        .attr('text-anchor', 'middle')
        .style('fill', '#f8fafc')
      ...
      group.selectAll<SVGTextElement, number>('text.doughnut-center-value')
        .data([total])
        .join('text')
        .attr('class', 'doughnut-center-value')
        .attr('x', centerX)
        .attr('y', centerY + 14)
        .attr('text-anchor', 'middle')
        .style('fill', 'rgba(226, 232, 240, 0.72)')
        .style('font-size', '12px')
    ...
    tooltip({ data: points, plotSize, seriesGroup, mouseX, mouseY }) {
      ...
      seriesGroup.selectAll<SVGPathElement, PieSegment>('path.doughnut-segment')
        .style('fill-opacity', 0.84)
        .style('stroke', '#101720');
      ...
      const [x, y] = pointOnCircle(centerX, centerY, (innerRadius + outerRadius) / 2, midAngle);
      return {
        data: segment.point,
        x,
        y,
...
const chart = createKChart<SegmentPoint>({
  ...
  title: { text: 'Market Share Doughnut Chart', align: 'left', fontSize: 18 },
  grid: { visible: false },
  legend: { visible: false },
  tooltip: { visible: true },
  axes: [],
  series: [createDoughnutSeries()]
});
