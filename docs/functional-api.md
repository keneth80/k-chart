...
`createGeoRegionMapSeries()`는 GeoJSON `FeatureCollection`을 받아 각 region을 SVG path로 렌더링합니다. 한국 시도, 세계 국가, 사내 공장 구역처럼 경계 데이터가 있는 지도형 시각화에 사용할 수 있습니다. `dataKey`와 `featureKey`가 같은 값을 가리키면 chart data가 GeoJSON feature에 매핑됩니다.
